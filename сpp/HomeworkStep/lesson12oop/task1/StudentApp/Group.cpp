...
void Group::DelStudent(int poz)//удалить студента 
{
	if (poz > 0 && poz <= size) //если позиция больше 0 и меньше либо равно размеру
	{
		Student* tmp = new Student[size - 1];
		int j = 0;
		for (int i = 0; i < size; i++)
		{
			if (i != poz - 1)//в новый массив копируем все кроме указаной позиции 
			{
